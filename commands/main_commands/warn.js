        ...
        pool.query(`SELECT * FROM guilds WHERE guild_id = ${message.guild.id}`, function(error, results) {
            ...
            if (!results[0]) return message.channel.send(`${message.guild.owner} doit configurer le serveur avant que vous puissiez avertir un utilisateur !`);                
                try {
                    ...
                    title: config.language[results[0].language ? results[0].language : "en"].phrases.warn.success_embed_title,
                    description: format(config.language[results[0].language ? results[0].language : "en"].phrases.warn.success_embed_description, `${member_warn}`),
                    timestamp: new Date()
                    }})
                    member_warn.send({embed: {
                        color: config.colors.warning,
                        title: config.language[results[0].language ? results[0].language : "en"].phrases.warn.warn_embed_user_title,
                        description: format(config.language[results[0].language ? results[0].language : "en"].phrases.warn.warn_embed_user_description, `${message.author}`, `*${reason_warn}*`, `*${message.guild.name}*`),
                        timestamp: new Date()
                    }})
                }
                catch(error) {
                    ...
                    title:  config.language[results[0].language ? results[0].language : "en"].phrases.warn.errors_embed_title,
                    description: format(config.language[results[0].language ? results[0].language : "en"].phrases.warn.errors_embed_description, `${member_warn}`, `${error}`)
                }});
            }
    })
