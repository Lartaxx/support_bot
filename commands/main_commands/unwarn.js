...
module.exports = class UnwarnCommand extends Command {
    ...
    run(message, {member_unwarn, number_unwarn, reason_unwarn}) {
        pool.query(`SELECT * FROM warns WHERE user_id = ${member_unwarn.id}`, function(err, results) {
            if (err) throw err;
            ...
            if ( Math.sign(results[0].nbr_warn) === -1 ) {
                ...
                let embed_errors_warn = new MessageEmbed()
                    ...
                    .setTitle(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.errors_embed_title)
                    .setDescription(format(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.errors_embed_description, number_unwarn, member_unwarn, results[0].nbr_warn - number_unwarn));
                ...
                .setColor(config.colors.errors)
                .setTitle(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.errors_embed_title)
                .setDescription(format(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.errors_embed_empty_description, member_unwarn))
                ...
                let embed_success_warn = new MessageEmbed()
                    ...
                    .setTitle(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.success_embed_title)
                    .setDescription(format(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.success_embed_description, `${message.author}`, `${number_unwarn}`, `${member_unwarn}`))
                    .addField(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.success_embed_field_title, `${reason_unwarn}`, false)
                    .setFooter(format(config.language[lang[0].language ? lang[0].language : "en"].phrases.unwarn.success_embed_footer, `${results[0].nbr_warn - number_unwarn}`))
                    .setTimestamp();
